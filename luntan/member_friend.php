...
$pagesize = 3;
//所有的数据条数
$total = mysqli_num_rows(mysqli_query($link,"SELECT tg_id FROM tg_friend WHERE (tg_touser = '{$_COOKIE['username']}') OR (tg_fromuser = '{$_COOKIE['username']}')"));
//数据库清零判断
if ($total == 0){
    $page_click = 1;
}else{
...
//从数据库中选择数据,获取结果集
$result = mysqli_query($link,"SELECT tg_id,tg_touser,tg_fromuser,tg_content,tg_date,tg_state FROM tg_friend WHERE tg_touser = '{$_COOKIE['username']}' OR tg_fromuser = '{$_COOKIE['username']}' ORDER BY tg_date DESC LIMIT $pagenum,$pagesize");
...
if ($_GET['action'] == 'delete' && isset($_POST['ids'])){
    ...
    if($row){
        if ($row['tg_uniqid'] == $_COOKIE['uniqid']){
            mysqli_query($link,"DELETE FROM tg_friend WHERE tg_id IN ({$clean['ids']})");
    ...
    <div id="member_main">
        ...
        <table cellspacing="1">
           ...
           <?php while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)) {?>
                    <tr>
                        <td><?php
                            if ($_COOKIE['username'] == $row['tg_touser']){
                                $friend = $row['tg_fromuser'];
                            }elseif($_COOKIE['username'] == $row['tg_fromuser']){
                                $friend = $row['tg_touser'];
                            }
                            echo $friend;?>
                        </td>
                        <td><a href="member_friend_detail.php?id=<?php echo $row['tg_id']?>" title="<?php echo $row['tg_content']?>"><?php echo title($row['tg_content'])?></a></td>
                        <td><?php echo $row['tg_date'];?></td>
                        <td>
                            <?php
                            if ($_COOKIE['username'] == $row['tg_touser']){
                                if ($row['tg_state'] == 0){
                                    echo '<span style="color: blue"><a href="member_friend_detail.php?action=check&id='.$row['tg_id'].'">您未验证!</a></span>';
                                }else{
                                    echo "<span style='color: green'>您已验证!</span>";
                                }
                            }elseif($_COOKIE['username'] == $row['tg_fromuser']){
                                if ($row['tg_state'] == 0){
                                    echo "<span style='color: red;'>他未验证!</span>";
                                }else {
                                    echo "<span style='color: green'>验证通过!</span>";
                                }
                            }
                            ?>
                        </td>
