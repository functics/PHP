...
//查询数据库
    $result = mysqli_query($link,"SELECT tg_id,tg_fromuser,tg_content,tg_date,tg_state FROM tg_message WHERE tg_id ='{$_GET['id']}' LIMIT 1");
    ...
    //已读设置
    if ($row['tg_state'] == 0){
        mysqli_query($link,"UPDATE tg_message SET tg_state = 1 WHERE tg_id ='{$_GET['id']}' LIMIT 1");
    }
...
if ($_GET['action'] == 'delete' && $_GET['id']){
    //验证私信是否合法
    $result = mysqli_query($link,"SELECT tg_id FROM tg_message WHERE tg_id ='{$_GET['id']}' LIMIT 1");
    $row = mysqli_fetch_array($result,MYSQLI_ASSOC);
    if ($row){
        //比对唯一标识符
        ...
        if ($row_uni){
            ...
            //删除单条私信
            mysqli_query($link,"DELETE FROM tg_message WHERE tg_id='{$_GET['id']}' LIMIT 1");
            if(mysqli_affected_rows($link) == 1){
                //数据库关闭
